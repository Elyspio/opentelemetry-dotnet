...
namespace Elyspio.OpenTelemetry.Technical.Helpers;
...
public static class Log
{
	...
	public sealed class LoggerInstance : IDisposable
	{
		...
		public LoggerInstance(ILogger logger, string method, string? arguments, LogLevel level, Activity? activity = null, bool autoExit = true, string className = "", bool logOnExit = true)
		{
			_level = level;
			Activity = activity;
			_autoExit = autoExit;
		...
		private void Enter()
		{
			...
			Activity?.AddBaggage("log-enter", message);
		...
		public void Exit(string? content = null)
		{
			...
			Activity?.AddBaggage("log-exit", message);
